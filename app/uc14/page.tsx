...
function ImpactBadge({ impact }: { impact: string }) {
  ...
  return (
    <span style={{ display: 'inline-block', background: bg, border: `1px solid ${color}`, color, fontWeight: 700, fontSize: '16px', letterSpacing: '1px', padding: '3px 8px', textTransform: 'uppercase' }}>
      {impact}
    </span>
  )
}
...
function AutomationBadge({ potential }: { potential: string }) {
  ...
  return (
    <span style={{ display: 'inline-block', background: bg, border: `1px solid ${color}22`, color, fontWeight: 600, fontSize: '16px', letterSpacing: '0.5px', padding: '2px 7px' }}>
...
function SatisfiabilityBadge({ value }: { value: Satisfiable }) {
  ...
  return (
    <span style={{ display: 'inline-block', background: c.bg, border: `1px solid ${c.color}`, color: c.color, fontWeight: 700, fontSize: '16px', letterSpacing: '1px', padding: '3px 8px', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
      {c.label}
    </span>
  )
}
...
function FieldTile({ field, status }: { field: string; status: string }) {
  ...
  return (
    <div style={{ background: bg, border: `1px solid ${border}`, padding: '8px 10px' }}>
      <div style={{ fontSize: '16px', fontWeight: 700, color, letterSpacing: '0.5px', textTransform: 'uppercase', marginBottom: '2px' }}>
        {status}
      </div>
      <div style={{ fontSize: '16px', color: '#374151', lineHeight: 1.3 }}>{FIELD_LABELS[field] || field}</div>
    </div>
  )
}
...
function ReframeStrip() {
  return (
    <div style={{ background: '#ffffff', borderBottom: '1px solid #e5e5e5', padding: '40px 80px' }}>
      <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#737373', textTransform: 'uppercase', marginBottom: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', maxWidth: '900px' }}>
        <div style={{ border: '1px solid #e5e5e5', padding: '24px 28px', background: '#fafafa', position: 'relative' }}>
          <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#737373', textTransform: 'uppercase', marginBottom: '14px' }}>
            Catalog audit asks
          </div>
          ...
          <p style={{ fontSize: '16px', color: '#525252', margin: '0 0 12px', lineHeight: 1.6 }}>
            Answer: yes, for 14 of 17 cosmetics PDPs.
          </p>
          <div style={{ background: '#fff5f5', border: '1px solid #fecaca', padding: '10px 14px' }}>
            <p style={{ fontSize: '16px', color: '#991b1b', margin: 0, lineHeight: 1.6 }}>
        ...
        <div style={{ border: '2px solid #0a0a0a', padding: '24px 28px', background: '#ffffff', position: 'relative' }}>
          <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#E2001A', textTransform: 'uppercase', marginBottom: '14px' }}>
            Intent audit asks
          </div>
          <p style={{ fontSize: '18px', fontWeight: 700, color: '#0a0a0a', lineHeight: 1.4, margin: '0 0 16px' }}>
          ...
          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '10px 14px' }}>
            <p style={{ fontSize: '16px', color: '#166534', margin: 0, lineHeight: 1.6 }}>
  ...
  return (
    <div style={{ background: '#ffffff', border: '1px solid #e5e5e5', padding: '24px 28px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '14px' }}>
        <div>
          <div style={{ fontSize: '16px', fontWeight: 700, color: '#0a0a0a', marginBottom: '4px' }}>{intent.label}</div>
          <div style={{ fontSize: '16px', fontWeight: 600, color: '#E2001A', fontStyle: 'italic', letterSpacing: '0.3px' }}>{intent.german_label}</div>
        </div>
        <SatisfiabilityBadge value={intent.satisfiable} />
      </div>

      <p style={{ fontSize: '16px', color: '#525252', lineHeight: 1.6, margin: '0 0 14px' }}>{intent.description}</p>

      {/* Example queries */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '16px' }}>
        {intent.example_queries.map(q => (
          <span key={q} style={{ fontSize: '16px', fontWeight: 600, padding: '3px 10px', background: '#f5f5f5', border: '1px solid #e5e5e5', color: '#0a0a0a', fontFamily: 'monospace' }}>
            {q}
          </span>
        ))}
      </div>
      ...
      <div style={{ background: '#f9f9f9', borderLeft: '3px solid #0a0a0a', padding: '10px 14px', marginBottom: '14px' }}>
        <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '1.5px', color: '#737373', textTransform: 'uppercase', marginBottom: '4px' }}>Search engine needs</div>
        <p style={{ fontSize: '16px', color: '#0a0a0a', margin: 0, lineHeight: 1.5 }}>{intent.required_field}</p>
      </div>

      {/* Gap insight */}
      <p style={{ fontSize: '16px', color: '#737373', lineHeight: 1.6, margin: '0 0 14px', borderLeft: '2px solid #E2001A', paddingLeft: '10px' }}>
        {intent.gap_insight}
      </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
        <div style={{ background: '#fafafa', border: '1px solid #f0f0f0', padding: '8px 12px' }}>
          <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '1px', color: '#737373', textTransform: 'uppercase', marginBottom: '2px' }}>Cosmetics</div>
          <div style={{ fontSize: '16px', fontWeight: 900, color: intent.coverage_cosmetics === '0 / 17' ? '#E2001A' : '#d97706', letterSpacing: '0px' }}>{intent.coverage_cosmetics}</div>
          <div style={{ fontSize: '16px', color: '#a3a3a3' }}>structured coverage</div>
        </div>
        <div style={{ background: '#fafafa', border: '1px solid #f0f0f0', padding: '8px 12px' }}>
          <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '1px', color: '#737373', textTransform: 'uppercase', marginBottom: '2px' }}>Mother &amp; Child</div>
          <div style={{ fontSize: '16px', fontWeight: 900, color: intent.coverage_mother_child === '0 / 18' ? '#E2001A' : '#d97706', letterSpacing: '0px' }}>{intent.coverage_mother_child}</div>
          <div style={{ fontSize: '16px', color: '#a3a3a3' }}>structured coverage</div>
        </div>
      </div>
    </div>
...
function GapMatrix() {
  return (
    <section style={{ background: '#0a0a0a', padding: '36px 48px', marginBottom: '0' }}>
      <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#E2001A', textTransform: 'uppercase', marginBottom: '6px' }}>Intent × Coverage Matrix</div>
      <p style={{ fontSize: '16px', color: '#737373', margin: '0 0 24px' }}>Can a user searching by this intent type find the right product? — structured coverage across 35 PDPs</p>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '16px' }}>
          <thead>
            <tr>
              {['Query intent', 'Cosmetics (17 PDPs)', 'Mother & Child (18 PDPs)', 'Satisfiable?', 'Search consequence'].map(h => (
                <th key={h} style={{ textAlign: 'left', padding: '10px 14px', borderBottom: '1px solid #262626', fontWeight: 700, fontSize: '16px', letterSpacing: '0.5px', color: '#737373', textTransform: 'uppercase', whiteSpace: 'nowrap', ...(h === 'Search consequence' ? { width: '33%' } : {}) }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {QUERY_INTENTS.map((intent, i) => (
              <tr key={intent.id} style={{ borderBottom: '1px solid #1a1a1a' }}>
                <td style={{ padding: '12px 14px' }}>
                  <div style={{ fontWeight: 700, color: '#ffffff', fontSize: '16px', marginBottom: '2px' }}>{intent.label}</div>
                  <div style={{ color: '#E2001A', fontSize: '16px', fontStyle: 'italic' }}>{intent.german_label}</div>
                </td>
                <td style={{ padding: '12px 14px', fontWeight: 700, color: intent.coverage_cosmetics === '0 / 17' ? '#E2001A' : '#d97706', fontSize: '16px', letterSpacing: '0px' }}>
                  {intent.coverage_cosmetics}
                </td>
                <td style={{ padding: '12px 14px', fontWeight: 700, color: intent.coverage_mother_child === '0 / 18' ? '#E2001A' : '#d97706', fontSize: '16px', letterSpacing: '0px' }}>
                  {intent.coverage_mother_child}
                </td>
                <td style={{ padding: '12px 14px' }}><SatisfiabilityBadge value={intent.satisfiable} /></td>
                <td style={{ padding: '12px 14px', color: '#a3a3a3', fontSize: '16px', lineHeight: 1.5, width: '33%' }}>{intent.search_consequence}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  return (
    <section style={{ background: '#ffffff', border: '1px solid #e5e5e5', marginBottom: '48px' }}>
      <div style={{ padding: '24px 32px', borderBottom: '1px solid #e5e5e5' }}>
        <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#737373', textTransform: 'uppercase', marginBottom: '4px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0' }}>
        ...
        <div style={{ padding: '28px 32px', borderRight: '1px solid #e5e5e5' }}>
          <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#d97706', textTransform: 'uppercase', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <div style={{ background: '#f9f9f9', border: '1px solid #e5e5e5', padding: '16px', marginBottom: '16px', fontFamily: 'monospace', fontSize: '16px', color: '#374151', lineHeight: 1.8 }}>
            <div style={{ color: '#737373', fontSize: '16px', marginBottom: '8px' }}>{'<div class="product-ingredients">'}</div>
            ...
            <div style={{ color: '#737373', fontSize: '16px', marginTop: '8px' }}>{'</div>'}</div>
          </div>
          <div style={{ background: '#fff5f5', border: '1px solid #fecaca', padding: '10px 14px' }}>
            <p style={{ fontSize: '16px', color: '#991b1b', margin: 0, lineHeight: 1.6 }}>
        ...
        <div style={{ padding: '28px 32px' }}>
          <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#16a34a', textTransform: 'uppercase', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#16a34a', display: 'inline-block' }} />
            What search needs — structured INCI array
          </div>
          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '16px', marginBottom: '16px', fontFamily: 'monospace', fontSize: '16px', color: '#374151', lineHeight: 1.8 }}>
            <div style={{ color: '#16a34a', fontSize: '16px', marginBottom: '8px' }}>ingredients: [</div>
            ...
            <div style={{ color: '#16a34a', fontSize: '16px' }}>]</div>
          </div>
          <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', padding: '10px 14px' }}>
            <p style={{ fontSize: '16px', color: '#166534', margin: 0, lineHeight: 1.6 }}>
      ...
      <main style={{ background: '#fafafa', minHeight: '100vh' }}>
        ...
        <div style={{ background: '#ffffff', borderBottom: '2px solid #0a0a0a', padding: '64px 80px 56px', display: 'flex', gap: '80px', alignItems: 'center' }}>
          <div style={{ flex: '0 0 auto', maxWidth: '520px' }}>
            <p style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '3px', color: '#E2001A', textTransform: 'uppercase', marginBottom: '10px' }}>Data Quality Audit — Search Intent Audit</p>
        ...
        <div style={{ background: '#fafafa', borderBottom: '1px solid #e5e5e5', padding: '48px 80px' }}>
          <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#737373', textTransform: 'uppercase', marginBottom: '6px' }}>Query Intent Taxonomy — 7 patterns</div>
          <p style={{ fontSize: '16px', color: '#525252', margin: '0 0 28px' }}>
        ...
        <div style={{ padding: '48px 80px 0' }}>
          ...
          <section style={{ background: '#ffffff', border: '1px solid #e5e5e5', marginBottom: '48px' }}>
            <div style={{ padding: '24px 32px', borderBottom: '1px solid #e5e5e5' }}>
              <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#737373', textTransform: 'uppercase', marginBottom: '4px' }}>Gaps by Field — ranked by query impact</div>
              <p style={{ fontSize: '16px', color: '#525252', margin: 0 }}>{GAP_REPORT.length} field-level gaps — with affected query intent types</p>
            </div>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '16px' }}>
                <thead>
                  <tr>
                    {['Field', 'Segment', 'Missing / Partial', 'Impact', 'Affects intent types', 'Why it matters', 'Data Source', 'Auto', 'How'].map(h => (
                      <th key={h} style={{ textAlign: 'left', padding: '10px 14px', background: '#f5f5f5', borderBottom: '2px solid #e5e5e5', fontWeight: 700, fontSize: '16px', letterSpacing: '0.5px', whiteSpace: 'nowrap' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {GAP_REPORT.map((gap, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #f0f0f0', background: i % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                      ...
                      <td style={{ padding: '12px 14px' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          {gap.affects_intents.map(id => (
                            <span key={id} style={{ fontSize: '16px', fontWeight: 600, padding: '2px 6px', background: '#fafafa', border: '1px solid #e5e5e5', color: '#525252', whiteSpace: 'nowrap' }}>
                              {INTENT_LABELS[id] || id}
                            </span>
                          ))}
                        </div>
                      ...
                      <td style={{ padding: '12px 14px', color: '#525252', fontSize: '16px', maxWidth: '180px', lineHeight: 1.4 }}>{gap.data_source}</td>
                      <td style={{ padding: '12px 14px' }}><AutomationBadge potential={gap.automation_potential} /></td>
                      <td style={{ padding: '12px 14px', color: '#525252', fontSize: '16px', maxWidth: '220px', lineHeight: 1.4 }}>{gap.automation_note}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <section style={{ background: '#ffffff', border: '1px solid #e5e5e5', marginBottom: '48px' }}>
            <div style={{ padding: '24px 32px', borderBottom: '1px solid #e5e5e5' }}>
              <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#737373', textTransform: 'uppercase', marginBottom: '4px' }}>Recommended Data Sources</div>
              <p style={{ fontSize: '16px', color: '#525252', margin: '0 0 4px' }}>
              ...
              <div style={{ display: 'flex', gap: '16px', marginTop: '12px' }}>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '16px', color: '#16a34a' }}>
                  <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#16a34a', display: 'inline-block' }} />
                  Automated — no editorial review per record
                </span>
                <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '16px', color: '#d97706' }}>
                  <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#d97706', display: 'inline-block' }} />
                ...
                <div key={i} style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px', marginBottom: '10px' }}>
                    <div>
                      ...
                      <div style={{ fontSize: '16px', color: '#737373', marginLeft: '16px' }}>{src.segments}</div>
                    </div>
                  </div>
                  <p style={{ fontSize: '16px', color: '#374151', lineHeight: 1.6, margin: '0 0 12px' }}>{src.description}</p>
                  <div style={{ background: '#f9f9f9', borderLeft: '3px solid #e5e5e5', padding: '10px 14px', marginBottom: '12px' }}>
                    <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '1px', color: '#737373', textTransform: 'uppercase', marginBottom: '4px' }}>How to integrate</div>
                    <p style={{ fontSize: '16px', color: '#525252', margin: 0, lineHeight: 1.6 }}>{src.how}</p>
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {src.fields.map(f => (
                      <span key={f} style={{
                        fontSize: '16px', fontWeight: 600, padding: '2px 8px',
          ...
          <section style={{ background: '#ffffff', border: '1px solid #e5e5e5', marginBottom: '64px' }}>
            <div style={{ padding: '24px 32px', borderBottom: '1px solid #e5e5e5' }}>
              <div style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '2px', color: '#737373', textTransform: 'uppercase', marginBottom: '4px' }}>Per-PDP Raw Field Audit — {PDP_EXTRACTIONS.length} PDPs</div>
              <p style={{ fontSize: '16px', color: '#525252', margin: '0 0 8px' }}>Field-level status across all 13 fields per PDP. Expand each row to see the breakdown.</p>
                ...
                return (
                  <details key={i} style={{ borderBottom: '1px solid #f0f0f0' }}>
                    <summary style={{ padding: '14px 32px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '16px', listStyle: 'none', background: i % 2 === 0 ? '#ffffff' : '#fafafa' }}>
                      <span style={{ fontSize: '16px', fontWeight: 700, letterSpacing: '1px', color: '#737373', textTransform: 'uppercase', minWidth: '100px' }}>{pdp.segment_label}</span>
                      <span style={{ fontSize: '16px', color: '#0a0a0a', fontFamily: 'monospace', flex: 1 }}>{displaySlug}</span>
                      <span style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        {missingCount > 0 && (
                          <span style={{ background: '#fff5f5', border: '1px solid #fecaca', color: '#991b1b', fontWeight: 700, fontSize: '16px', padding: '2px 8px' }}>
                            {missingCount} missing
                          </span>
                        )}
                        {partialCount > 0 && (
                          <span style={{ background: '#fffbeb', border: '1px solid #fde68a', color: '#92400e', fontWeight: 700, fontSize: '16px', padding: '2px 8px' }}>
                            {partialCount} partial
                          </span>
                        )}
                        {missingCount === 0 && partialCount === 0 && (
                          <span style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', color: '#166534', fontWeight: 700, fontSize: '16px', padding: '2px 8px' }}>
                            complete
                          </span>
                        )}
                      </span>
                    ...
                    <div style={{ padding: '16px 32px 24px', background: '#f9f9f9', borderTop: '1px solid #f0f0f0' }}>
                      <div style={{ marginBottom: '10px' }}>
                        <a href={pdp.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: '16px', color: '#737373', wordBreak: 'break-all' }}>{pdp.url}</a>
                      </div>
